 ...
 */
// Replace the Decrypt function in common/crypto.cpp with this corrected version:
    ...
    size_t* data_size)
{
    ...
    {
        TRACE_ENCLAVE("EVP_PKEY_decrypt_init failed!");
        goto exit;
    }
    ...
    if (!EVP_PKEY_decrypt(
            ...
            encrypted_data_size))
    {
        TRACE_ENCLAVE("EVP_PKEY_decrypt (size query) failed!");
        goto exit;
    }
    ...
    {
        TRACE_ENCLAVE("Caller's buffer too small! Need %zu bytes, got %zu bytes", temp_size, *data_size);
        *data_size = temp_size; // Tell caller how much space is needed
        goto exit;
    }
    
    // Allocate temporary buffer
    temp_buffer = (uint8_t*)OPENSSL_malloc(temp_size);
    if (!temp_buffer)
    {
        TRACE_ENCLAVE("OPENSSL_malloc failed!");
        goto exit;
    ...
    // Second call: perform actual decryption
    if (!EVP_PKEY_decrypt(
            ctx,
            temp_buffer,
            &temp_size,
