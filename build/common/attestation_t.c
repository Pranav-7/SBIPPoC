...
static void ecall_set_host_public_key(
    uint8_t* input_buffer,
    size_t input_buffer_size,
    uint8_t* output_buffer,
    size_t output_buffer_size,
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    set_host_public_key_args_t* _pargs_in = (set_host_public_key_args_t*)input_buffer;
    set_host_public_key_args_t* _pargs_out = (set_host_public_key_args_t*)output_buffer;
    ...
    /* Set in and in-out pointers. */
    if (_pargs_in->host_public_key)
        OE_SET_IN_POINTER(host_public_key, 1, sizeof(pem_key_t), pem_key_t*);
    if (_pargs_in->host_public_key && _pargs_in->host_public_key->buffer)
        OE_SET_IN_POINTER(host_public_key->buffer, 1, _pargs_in->host_public_key->size, uint8_t*);
    ...
    /* Call user function. */
    _pargs_out->oe_retval = set_host_public_key(
        _pargs_in->host_public_key);
...
static void ecall_process_encrypted_defi(
    uint8_t* input_buffer,
    size_t input_buffer_size,
    uint8_t* output_buffer,
    size_t output_buffer_size,
    size_t* output_bytes_written)
{
    ...
    /* Prepare parameters. */
    process_encrypted_defi_args_t* _pargs_in = (process_encrypted_defi_args_t*)input_buffer;
    process_encrypted_defi_args_t* _pargs_out = (process_encrypted_defi_args_t*)output_buffer;
    ...
    /* Set in and in-out pointers. */
    if (_pargs_in->encrypted_defi_package)
        OE_SET_IN_POINTER(encrypted_defi_package, 1, sizeof(message_t), message_t*);
    if (_pargs_in->encrypted_defi_package && _pargs_in->encrypted_defi_package->data)
        OE_SET_IN_POINTER(encrypted_defi_package->data, 1, _pargs_in->encrypted_defi_package->size, uint8_t*);

    /* Set out and in-out pointers. */
    /* In-out parameters are copied to output buffer. */
    if (_pargs_in->response)
        OE_SET_OUT_POINTER(response, 1, sizeof(message_t), message_t*);
    ...
    /* Call user function. */
    _pargs_out->oe_retval = process_encrypted_defi(
        _pargs_in->encrypted_defi_package,
        _pargs_in->response);

    /* Compute the size for the deep-copy out buffer. */
    if (_pargs_in->response && _pargs_in->response->data)
        OE_ADD_ARG_SIZE(_deepcopy_out_buffer_size, 1, _pargs_in->response->size);
    ...
    /* Serialize the deep-copied content into the buffer. */
    if (_pargs_in->response && _pargs_in->response->data)
        OE_WRITE_DEEPCOPY_OUT_PARAM(_pargs_in->response->data, 1, _pargs_in->response->size);
    ...
    *output_bytes_written = _output_buffer_offset;
    ...
    if (_pargs_in->response)
    {
        for (size_t _i_1 = 0; _i_1 < 1; _i_1++)
        {
            free(_pargs_in->response[_i_1].data);
        }
    }
...
oe_ecall_func_t oe_ecalls_table[] = {
    ...
    (oe_ecall_func_t) ecall_set_host_public_key,
    (oe_ecall_func_t) ecall_process_encrypted_defi,
    ...
    tdx_quote_collateral_t* collateral)
{
    ...
    /* Call host function. */
    if ((_result = oe_call_host_function(
             attestation_fcn_id_oe_get_tdx_quote_verification_collateral_ocall,
             _input_buffer,
             _input_buffer_size,
             _output_buffer,
             _output_buffer_size,
    ...
    uint32_t* p_supplemental_data_size_out)
{
    ...
    /* Call host function. */
    if ((_result = oe_call_host_function(
             attestation_fcn_id_oe_verify_tdx_quote_ocall,
             _input_buffer,
             _input_buffer_size,
             _output_buffer,
             _output_buffer_size,
